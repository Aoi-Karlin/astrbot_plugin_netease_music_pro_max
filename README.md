...
1. 在 AstrBot 的插件商店中搜索 `netease_music_pro_max` 并安装。
2. 或者，直接将本项目克隆到您的 AstrBot `data/plugins` 目录下。

### 配置

安装并重启 AstrBot 后，在网页后台的 **插件配置** -> **`netease_music_pro_max`** 中进行设置：
...
- **本插件修正**：
  修复了原插件中无法使用Cookies导致的无法播放VIP歌曲，修复了原插件使用指令会导致的两次回复。
## 💖 致谢

感谢 [AstrBot](https://github.com/AstrBotDevs/AstrBot) 提供了如此强大的机器人框架。

感谢 [NachoCrazy](https://github.com/NachoCrazy/netease-music-astrbot-plugin) 制作的原版插件

---
*Made with ❤️ by NachoCrazy*
*and forked by azured*
